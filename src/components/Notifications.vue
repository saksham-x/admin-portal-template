...
const isAllMarkRead = computed(
  () => props.notifications.some((item: Notification) => item.isSeen === false),
)

const markAllReadOrUnread = () => {
  const allNotificationsIds = props.notifications.map((item: Notification) => item.id)
...
const totalUnreadNotifications = computed(
  () => props.notifications.filter((item: Notification) => !item.isSeen).length,
)
